...
describe('calculateMoldSellingPrice', () => {
  ...
  it('处理不同利润率的情况', () => {
    ...
    // 无利润情况（利润率0%）
    expect(calculateMoldSellingPrice(materialCost, processingCost, 0)).toBe(15000);
    
    // 100%利润
    expect(calculateMoldSellingPrice(materialCost, processingCost, 1.0)).toBe(30000);
    
    // 负利润（亏本50%）
    expect(calculateMoldSellingPrice(materialCost, processingCost, -0.5)).toBe(7500);
  });

  it('处理异常情况', () => {
    // 利润率不能低于 -1（-100%）
    expect(() => calculateMoldSellingPrice(10000, 5000, -1.1))
      .toThrow('利润率不能低于 -100%');
    
    // 成本不能为负数
    expect(() => calculateMoldSellingPrice(-10000, 5000, 0.3))
      .toThrow('材料成本不能为负数');
    expect(() => calculateMoldSellingPrice(10000, -5000, 0.3))
      .toThrow('加工成本不能为负数');
  });
