import { calculateProductMaterialCost, calculateProductTotalCost } from '../cost';
...
describe('产品成本计算', () => {
  describe('calculateProductMaterialCost - 单件产品材料成本', () => {
    ...
    it('处理负数输入', () => {
      expect(() => calculateProductMaterialCost(-100, 0.91, 15))
        .toThrow('产品体积不能为负数');
      expect(() => calculateProductMaterialCost(100, -0.91, 15))
        .toThrow('材料密度不能为负数');
      expect(() => calculateProductMaterialCost(100, 0.91, -15))
  ...
  describe('calculateProductTotalCost - 总产品成本', () => {
    ...
    it('处理非整数数量', () => {
      expect(() => calculateProductTotalCost(100.5, 1.365))
        .toThrow('产品数量必须为整数');
    });

    it('处理负数输入', () => {
      expect(() => calculateProductTotalCost(-100, 1.365))
        .toThrow('产品数量不能为负数');
      expect(() => calculateProductTotalCost(100, -1.365))
        .toThrow('单件成本不能为负数');
    });
