...
describe('优化策略', () => {
  describe('ternarySearchForOptimalCavity', () => {
    ...
    it('处理单调递增的成本函数', () => {
      ...
      const result = ternarySearchForOptimalCavity(increasingCostFunction, 1, 32);
      // 当成本单调递增时，最优解应该在左边界
    ...
    it('处理单调递减的成本函数', () => {
      // 模拟成本随穴数增加而单调递减的情况
      const decreasingCostFunction = (cavity: number): number => 100000 / cavity;
      
      const result = ternarySearchForOptimalCavity(decreasingCostFunction, 1, 32);
      // 当成本单调递减时，最优解应该在右边界
    ...
    it('验证搜索过程的收敛性', () => {
      ...
      const result = ternarySearchForOptimalCavity(quadraticCostFunction, 1, 32);
      // 验证是否收敛到真实的最优解附近
