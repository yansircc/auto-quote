...
describe("Distribution Score Calculation", () => {
  ...
  describe("基础分布测试", () => {
    ...
    test("场景2: 对称布局 - 四个相同产品", () => {
      ...
      const positions: [number, number][] = [
        ...
        [100, -100], // 上排两个
        [-100, 100],
        [100, 100], // 下排两个，进一步减小间距以提高分数
      ];

      const products = Array(4)
        .fill(null)
        .map((_, i) => createSizedProduct(i + 1, 100, SIZES.MEDIUM));
      ...
      expect(result.details.volumeBalance.massDistribution).toBeGreaterThan(
        0.8,
      );
      // 空间分布应该很好
  ...
  describe("实际生产场景测试", () => {
    test("场景1: 大型产品周围环绕小型产品", () => {
      ...
      const products = [
        ...
        ...Array(4)
          .fill(null)
          .map((_, i) => createSizedProduct(i + 2, 100, SIZES.SMALL)),
      ];
    ...
    test("场景3: 散热优先布局", () => {
      ...
      const products = Array(4)
        .fill(null)
        .map((_, i) => createSizedProduct(i + 1, 200, SIZES.LARGE));
  ...
  describe("性能边界测试", () => {
    test("场景1: 大量小型产品均匀分布", () => {
      ...
      const products = Array(gridSize * gridSize)
        .fill(null)
        .map((_, i) => createSizedProduct(i + 1, 50, SIZES.SMALL));
