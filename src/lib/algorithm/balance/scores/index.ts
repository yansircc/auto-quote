 ...
 * 基于可用数据计算置信度
 */
function calculateConfidence(products: NormalizedProduct[]): number {
  const dataPoints = products.filter(
    (p) => p.volume != null && p.surfaceArea != null,
  ).length;
...
function calculateBalanceScore(
  ...
  const geometryScore = new GeometryScorer().calculateScore(products);
